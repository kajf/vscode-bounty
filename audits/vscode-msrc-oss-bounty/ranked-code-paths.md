...
| 3 | Manual task execution and variable resolution | `abstractTaskService.ts`, `terminalTaskSystem.ts`, configuration resolver service, task inputs | Command strings, `${...}` substitutions, shell/process mode, env/cwd and task inputs are classic injection surfaces. | Source checkout available; deeper trace pending. Benign fixture added. |
